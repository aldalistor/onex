...
function WindowBody({ window, onSave, records, isSaving }: { window: WindowRecord; onSave: () => void; records: Array<{ docNo?: string; status?: string; grandTotal?: number | string }>; isSaving: boolean }) {
  ...
  return (
    <div className="legacy-window-body">
      <div className="legacy-form-toolbar">
        <button title="جديد"><FilePlus2 size={15} /></button><button title="حفظ المسودة" onClick={onSave} disabled={isSaving}><Archive size={15} /></button><button title="طباعة"><Printer size={15} /></button><span className="toolbar-separator" /><button title="بحث"><Search size={15} /></button><button title="مساعدة"><CircleHelp size={15} /></button>
      </div>
...
export default function WindowWorkbench() {
  ...
  function saveDraft() {
    if (!active || active.id !== "ARST004") return;
    const customer = masterData.data?.customers?.[0];
    const warehouse = masterData.data?.warehouses?.[0];
    const item = masterData.data?.items?.[0];
    if (!customer || !warehouse || !item) return;
    createInvoice.mutate({ docNo: `${active.id}-${Date.now()}`, customerId: customer.id, warehouseId: warehouse.id, actor: "workbench.user", lines: [{ itemId: item.id, quantity: "1", unitPrice: "0", taxAmount: "0" }] });
  }
