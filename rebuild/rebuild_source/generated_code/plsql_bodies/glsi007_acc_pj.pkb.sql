...
CREATE OR REPLACE PACKAGE BODY RB_GLSI007_ACC_PJ_3E5D2 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'glsi007_acc_pj') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for glsi007_acc_pj');
    END IF;
