-- GENERATED CODE — PMS_IMPXLS_1.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_PMS_IMPXLS_1_5D338 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'PMS_IMPXLS_1') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for PMS_IMPXLS_1');
    END IF;
