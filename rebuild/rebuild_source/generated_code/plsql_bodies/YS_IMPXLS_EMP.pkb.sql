-- GENERATED CODE — YS_IMPXLS_EMP.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_YS_IMPXLS_EMP_16317 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'YS_IMPXLS_EMP') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for YS_IMPXLS_EMP');
    END IF;
