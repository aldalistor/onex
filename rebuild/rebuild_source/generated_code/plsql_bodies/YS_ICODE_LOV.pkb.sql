-- GENERATED CODE — YS_ICODE_LOV.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_YS_ICODE_LOV_D00C9 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'YS_ICODE_LOV') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for YS_ICODE_LOV');
    END IF;
