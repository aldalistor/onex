-- GENERATED CODE — HRSTRVLT001.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_HRSTRVLT001_8E81C IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'HRSTRVLT001') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for HRSTRVLT001');
    END IF;
