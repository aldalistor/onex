-- GENERATED CODE — REM_ALRTS001.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_REM_ALRTS001_BFEF1 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'REM_ALRTS001') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for REM_ALRTS001');
    END IF;
