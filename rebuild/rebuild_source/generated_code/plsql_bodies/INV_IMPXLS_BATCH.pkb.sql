-- GENERATED CODE — INV_IMPXLS_BATCH.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_INV_IMPXLS_BATCH_EC298 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'INV_IMPXLS_BATCH') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for INV_IMPXLS_BATCH');
    END IF;
