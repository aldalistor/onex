-- GENERATED CODE — ARSIMPXLS_CST.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_ARSIMPXLS_CST_9560A IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ARSIMPXLS_CST') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ARSIMPXLS_CST');
    END IF;
