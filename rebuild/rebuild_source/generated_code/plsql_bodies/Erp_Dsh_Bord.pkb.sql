-- GENERATED CODE — Erp_Dsh_Bord.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_ERP_DSH_BORD_65495 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'Erp_Dsh_Bord') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for Erp_Dsh_Bord');
    END IF;
