-- GENERATED CODE — ERP_CODE_TRNS.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_ERP_CODE_TRNS_DC105 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ERP_CODE_TRNS') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ERP_CODE_TRNS');
    END IF;
