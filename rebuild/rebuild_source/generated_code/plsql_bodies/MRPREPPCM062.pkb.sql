-- GENERATED CODE — MRPREPPCM062.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_MRPREPPCM062_E42F1 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'MRPREPPCM062') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for MRPREPPCM062');
    END IF;
