-- GENERATED CODE — REMIMPXLS_REAL.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_REMIMPXLS_REAL_AD6C8 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'REMIMPXLS_REAL') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for REMIMPXLS_REAL');
    END IF;
