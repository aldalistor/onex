-- GENERATED CODE — mrprepmps0062.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_MRPREPMPS0062_2AE1B IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'mrprepmps0062') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for mrprepmps0062');
    END IF;
