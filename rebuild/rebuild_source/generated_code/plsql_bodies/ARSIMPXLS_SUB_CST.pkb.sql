-- GENERATED CODE — ARSIMPXLS_SUB_CST.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_ARSIMPXLS_SUB_CST_A9E21 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ARSIMPXLS_SUB_CST') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ARSIMPXLS_SUB_CST');
    END IF;
  ...
  BEGIN
    RETURN g_status;
  END status;
END RB_ARSIMPXLS_SUB_CST_A9E21;
/
