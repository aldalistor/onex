-- GENERATED CODE — ERP_DBA_DFLT_DATA.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_ERP_DBA_DFLT_DATA_9D746 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ERP_DBA_DFLT_DATA') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ERP_DBA_DFLT_DATA');
    END IF;
  ...
  BEGIN
    RETURN g_status;
  END status;
END RB_ERP_DBA_DFLT_DATA_9D746;
/
