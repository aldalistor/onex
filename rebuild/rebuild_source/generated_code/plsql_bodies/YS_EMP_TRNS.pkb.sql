-- GENERATED CODE — YS_EMP_TRNS.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_YS_EMP_TRNS_3E1D8 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'YS_EMP_TRNS') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for YS_EMP_TRNS');
    END IF;
