-- GENERATED CODE — HRS_IMPXLS_LOAN.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_HRS_IMPXLS_LOAN_995F1 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'HRS_IMPXLS_LOAN') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for HRS_IMPXLS_LOAN');
    END IF;
