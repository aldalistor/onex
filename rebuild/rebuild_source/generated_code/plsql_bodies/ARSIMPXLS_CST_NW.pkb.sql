-- GENERATED CODE — ARSIMPXLS_CST_NW.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_ARSIMPXLS_CST_NW_CD731 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ARSIMPXLS_CST_NW') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ARSIMPXLS_CST_NW');
    END IF;
