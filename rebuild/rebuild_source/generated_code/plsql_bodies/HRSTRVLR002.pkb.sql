-- GENERATED CODE — HRSTRVLR002.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_HRSTRVLR002_946FA IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'HRSTRVLR002') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for HRSTRVLR002');
    END IF;
