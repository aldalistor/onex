...
CREATE OR REPLACE PACKAGE BODY RB_ERP_DBA_DFLT_DATA_LNG_B0D9F IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ERP_DBA_DFLT_DATA_LNG') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ERP_DBA_DFLT_DATA_LNG');
    END IF;
