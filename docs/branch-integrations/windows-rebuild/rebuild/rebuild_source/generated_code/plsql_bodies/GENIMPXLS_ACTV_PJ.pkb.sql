-- GENERATED CODE — GENIMPXLS_ACTV_PJ.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_GENIMPXLS_ACTV_PJ_0FE9F IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'GENIMPXLS_ACTV_PJ') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for GENIMPXLS_ACTV_PJ');
    END IF;
  ...
  BEGIN
    RETURN g_status;
  END status;
END RB_GENIMPXLS_ACTV_PJ_0FE9F;
/
