-- GENERATED CODE — MRPREPSFC021.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_MRPREPSFC021_7F4BD IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'MRPREPSFC021') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for MRPREPSFC021');
    END IF;
