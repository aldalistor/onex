-- GENERATED CODE — HRS_RPRT_EMP.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_HRS_RPRT_EMP_1C0B3 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'HRS_RPRT_EMP') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for HRS_RPRT_EMP');
    END IF;
