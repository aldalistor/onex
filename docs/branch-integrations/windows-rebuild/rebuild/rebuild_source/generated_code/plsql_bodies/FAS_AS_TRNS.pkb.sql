-- GENERATED CODE — FAS_AS_TRNS.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_FAS_AS_TRNS_3E63E IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'FAS_AS_TRNS') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for FAS_AS_TRNS');
    END IF;
