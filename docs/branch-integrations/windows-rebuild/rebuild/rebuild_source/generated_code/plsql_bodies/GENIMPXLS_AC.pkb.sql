-- GENERATED CODE — GENIMPXLS_AC.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_GENIMPXLS_AC_F44B4 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'GENIMPXLS_AC') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for GENIMPXLS_AC');
    END IF;
