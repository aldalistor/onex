...
CREATE OR REPLACE PACKAGE BODY RB_YS_FAS_FLTR_E4B7E IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'YS_FAS_FLTR') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for YS_FAS_FLTR');
    END IF;
