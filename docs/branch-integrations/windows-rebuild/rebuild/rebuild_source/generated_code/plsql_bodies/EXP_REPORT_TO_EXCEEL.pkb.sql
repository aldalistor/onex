...
CREATE OR REPLACE PACKAGE BODY RB_EXP_REPORT_TO_EXCEEL_5D757 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'EXP_REPORT_TO_EXCEEL') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for EXP_REPORT_TO_EXCEEL');
    END IF;
