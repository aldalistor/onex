-- GENERATED CODE — FAS_IMP_XLS_ASSETS.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_FAS_IMP_XLS_ASSETS_F6A4D IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'FAS_IMP_XLS_ASSETS') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for FAS_IMP_XLS_ASSETS');
    END IF;
  ...
  BEGIN
    RETURN g_status;
  END status;
END RB_FAS_IMP_XLS_ASSETS_F6A4D;
/
