-- GENERATED CODE — HRSR014_NEW.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_HRSR014_NEW_2F446 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'HRSR014_NEW') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for HRSR014_NEW');
    END IF;
