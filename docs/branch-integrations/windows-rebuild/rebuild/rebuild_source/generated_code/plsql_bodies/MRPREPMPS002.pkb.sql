-- GENERATED CODE — MRPREPMPS002.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_MRPREPMPS002_2C89F IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'MRPREPMPS002') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for MRPREPMPS002');
    END IF;
