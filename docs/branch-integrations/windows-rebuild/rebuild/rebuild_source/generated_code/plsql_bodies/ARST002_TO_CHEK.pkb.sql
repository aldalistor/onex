...
CREATE OR REPLACE PACKAGE BODY RB_ARST002_TO_CHEK_539EA IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ARST002_TO_CHEK') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ARST002_TO_CHEK');
    END IF;
