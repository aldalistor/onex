-- GENERATED CODE — FAS_IMP_XLS_GRP.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_FAS_IMP_XLS_GRP_1B1BF IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'FAS_IMP_XLS_GRP') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for FAS_IMP_XLS_GRP');
    END IF;
