...
CREATE OR REPLACE PACKAGE BODY RB_ERP_UPDATE_NEW_VERSION_8A83 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ERP_Update_New_Version6') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ERP_Update_New_Version6');
    END IF;
