...
CREATE OR REPLACE PACKAGE BODY RB_MRPSFC023_A2403 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'MRPSFC023') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for MRPSFC023');
    END IF;
