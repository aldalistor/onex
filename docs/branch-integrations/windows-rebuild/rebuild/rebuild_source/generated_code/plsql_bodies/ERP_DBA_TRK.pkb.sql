...
CREATE OR REPLACE PACKAGE BODY RB_ERP_DBA_TRK_ABD0F IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'ERP_DBA_TRK') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for ERP_DBA_TRK');
    END IF;
