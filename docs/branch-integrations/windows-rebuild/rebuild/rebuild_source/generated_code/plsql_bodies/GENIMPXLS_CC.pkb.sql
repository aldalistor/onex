-- GENERATED CODE — GENIMPXLS_CC.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_GENIMPXLS_CC_9CF1D IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'GENIMPXLS_CC') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for GENIMPXLS_CC');
    END IF;
