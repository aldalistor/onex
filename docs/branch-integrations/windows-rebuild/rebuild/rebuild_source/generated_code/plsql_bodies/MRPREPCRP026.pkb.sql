-- GENERATED CODE — MRPREPCRP026.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_MRPREPCRP026_DD52C IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'MRPREPCRP026') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for MRPREPCRP026');
    END IF;
