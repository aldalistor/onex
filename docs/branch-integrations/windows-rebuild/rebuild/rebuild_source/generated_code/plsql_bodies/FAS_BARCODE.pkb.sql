-- GENERATED CODE — FAS_BARCODE.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_FAS_BARCODE_33AAB IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'FAS_BARCODE') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for FAS_BARCODE');
    END IF;
