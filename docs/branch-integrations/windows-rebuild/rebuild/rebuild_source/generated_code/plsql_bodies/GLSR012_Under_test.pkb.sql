-- GENERATED CODE — GLSR012_Under_test.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_GLSR012_UNDER_TEST_26B5F IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'GLSR012_Under_test') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for GLSR012_Under_test');
    END IF;
  ...
  BEGIN
    RETURN g_status;
  END status;
END RB_GLSR012_UNDER_TEST_26B5F;
/
