-- GENERATED CODE — MRPREPSGS012.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_MRPREPSGS012_F0A96 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'MRPREPSGS012') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for MRPREPSGS012');
    END IF;
