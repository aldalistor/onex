...
CREATE OR REPLACE PACKAGE BODY RB_MRPIMP_ITEMS_XLS_108EC IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'MRPIMP_ITEMS_XLS') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for MRPIMP_ITEMS_XLS');
    END IF;
