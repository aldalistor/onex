-- GENERATED CODE — HRSALNCR002.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_HRSALNCR002_C075E IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'HRSALNCR002') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for HRSALNCR002');
    END IF;
