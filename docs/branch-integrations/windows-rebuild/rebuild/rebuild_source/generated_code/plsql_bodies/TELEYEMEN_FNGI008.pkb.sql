-- GENERATED CODE — TELEYEMEN_FNGI008.fmx
...
CREATE OR REPLACE PACKAGE BODY RB_TELEYEMEN_FNGI008_EBF42 IS
  g_status VARCHAR2(30) := 'NEW';

  PROCEDURE initialize(p_form_name IN VARCHAR2 DEFAULT 'TELEYEMEN_FNGI008') IS
  BEGIN
    g_status := 'READY';
  ...
  BEGIN
    IF NOT is_valid(p_record_key) THEN
      RAISE_APPLICATION_ERROR(-20001, 'Validation failed for TELEYEMEN_FNGI008');
    END IF;
  ...
  BEGIN
    RETURN g_status;
  END status;
END RB_TELEYEMEN_FNGI008_EBF42;
/
